...
- **Usage — RESOLVED 2026-06-07 (read-only trace):** Cards does **not** call this — Cards uses its own JWT-gated `ocr-licence` (Claude Vision). Shell's `ocr-parse` has **one caller**: `eq-shell/src/pages/ocr/LicenceOcrPage.tsx:372`, which fetches with `credentials: 'include'` (sends the session cookie). That page is the route `onboarding/licence`, mounted under `/:tenantSlug/*` inside `<RequireSession>` (App.tsx:474-480) — i.e. **fully authenticated** (a logged-in operator scanning a worker's licence, NOT public self-signup). So gating the endpoint **will not break the only legitimate caller** — it's already logged in and already sending the cookie.
- **Fix (now safe + well-defined):** verify the Supabase session/JWT at the top of `ocr-parse.ts` (reject if absent) — mirrors what the other shell functions already do — and restrict the OPTIONS `Access-Control-Allow-Origin: *` to the shell origin. Low risk because the caller is authenticated.
- **Fix class:** NEEDS-APPROVAL (auth-flow change) + must be done in the eq-shell repo. To avoid disturbing the concurrent `claude/c3-auth-spike` checkout, implement in an isolated worktree off `main`.
- **Belt-and-suspenders (Royce, ~5 min, no code):** set a Google Cloud Document AI quota cap + budget alert so even an unforeseen gap is bounded.
