...
export function FieldEditor({ field, existing, onChange, compact = true, inputRef, pendingValue }: Props) {
  ...
  const confirmSuggestion = () => {
    ...
    // Same path as a typed value: update local draft + commit to queue.
    setDraft(pendingValue)
    triggerFlash()
    // Bypass commit's prev-comparison check — explicitly write the value.
    // Use direct onChange so we don't depend on closure-captured 'flagged'/'notes'
    // potentially holding stale values from a prior render cycle.
