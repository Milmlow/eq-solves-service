...
export async function proxy(request: NextRequest) {
  ...
  // Fetch profile once for admin-route check and deactivated-user check.
  // Admin routes need role + is_active; other protected routes only need is_active.
  if (pathname.startsWith('/admin') || (!isPublic && !isAalExempt)) {
    ...
    // Admin-only routes — role check after deactivation check.
