...
export async function createCheckAction(formData: FormData) {
  try {
    ...
    // 7. Notification if assigned (fire-and-forget — never blocks the primary action)
...
export async function updateCheckAction(id: string, formData: FormData) {
  try {
    ...
    // Create notification if assigned_to changed (fire-and-forget)
...
export async function completeCheckAction(id: string) {
  try {
    ...
    // Create notification to assigned technician's supervisor (fire-and-forget)
