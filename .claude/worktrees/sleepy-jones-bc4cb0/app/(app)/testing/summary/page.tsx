...
export default async function TestingSummaryPage({
  ...
  // Batch-fetch all ACB and NSX tests in two queries instead of one per check.
  const allCheckIds = (checks ?? []).map(c => c.id as string)
  const acbCheckIds = (checks ?? []).filter(c => c.check_type === 'acb').map(c => c.id as string)
  const nsxCheckIds = (checks ?? []).filter(c => c.check_type === 'nsx').map(c => c.id as string)

  type AcbTestRow = { id: string; asset_id: string; testing_check_id: string; step1_status: string | null; step2_status: string | null; step3_status: string | null; assets: unknown }
  type NsxTestRow = { id: string; asset_id: string; testing_check_id: string; step1_status: string | null; step2_status: string | null; step3_status: string | null; overall_result: string | null; assets: unknown }

  const [{ data: allAcbTests }, { data: allNsxTests }] = await Promise.all([
    acbCheckIds.length > 0
      ? supabase.from('acb_tests').select('id, asset_id, testing_check_id, step1_status, step2_status, step3_status, assets(id, name, asset_type, serial_number)').in('testing_check_id', acbCheckIds).eq('is_active', true)
      : Promise.resolve({ data: [] as AcbTestRow[] }),
    nsxCheckIds.length > 0
      ? supabase.from('nsx_tests').select('id, asset_id, testing_check_id, step1_status, step2_status, step3_status, overall_result, assets(id, name, asset_type, serial_number)').in('testing_check_id', nsxCheckIds).eq('is_active', true)
      : Promise.resolve({ data: [] as NsxTestRow[] }),
  ])

  // Group tests by testing_check_id
  const acbByCheck: Record<string, AcbTestRow[]> = {}
  for (const t of (allAcbTests ?? []) as AcbTestRow[]) {
    const key = t.testing_check_id
    if (!acbByCheck[key]) acbByCheck[key] = []
    acbByCheck[key].push(t)
  }
  const nsxByCheck: Record<string, NsxTestRow[]> = {}
  for (const t of (allNsxTests ?? []) as NsxTestRow[]) {
    const key = t.testing_check_id
    if (!nsxByCheck[key]) nsxByCheck[key] = []
    nsxByCheck[key].push(t)
  }

  void allCheckIds // used above; silence unused-var lint
  ...
  for (const check of checks ?? []) {
    ...
    if (check.check_type === 'acb') {
      for (const t of acbByCheck[check.id as string] ?? []) {
      ...
      for (const t of nsxByCheck[check.id as string] ?? []) {
        const assetRaw = t.assets as unknown
        const asset = (Array.isArray(assetRaw) ? assetRaw[0] : assetRaw) as { id: string; name: string; asset_type: string; serial_number: string | null } | null
        const pct = acbProgress(t)
        const st = acbStatus(t)
