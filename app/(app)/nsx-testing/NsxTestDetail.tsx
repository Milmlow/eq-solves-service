...
import { toggleNsxTestActiveAction, createNsxReadingAction, deleteNsxReadingAction } from './actions'
import { formatDate, formatNsxTestResult, formatNsxTestType } from '@/lib/utils/format'
...
export function NsxTestDetail({
  ...
  async function handleToggleActive() {
    setLoading(true)
    const result = await toggleNsxTestActiveAction(test.id, !test.is_active)
    setLoading(false)
    if (result.success) {
      onClose()
    } else {
  ...
  async function handleAddReading(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await createNsxReadingAction(test.id, formData)
    setLoading(false)
    if (result.success) {
  ...
  async function handleDeleteReading(readingId: string) {
    const result = await deleteNsxReadingAction(readingId)
    if (!result.success) {
  ...
  return (
    <SlidePanel open={open} onClose={onClose} title={`${assetName} — NSX Test`}>
      <div className="space-y-4">
        {/* Header */}
        <div className="flex items-center justify-between">
          <StatusBadge status={resultToBadge(test.overall_result)} label={formatNsxTestResult(test.overall_result)} />
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <dd className="text-eq-ink mt-1">{formatNsxTestType(test.test_type)}</dd>
          </div>
          <div>
            <dt className="text-xs font-bold text-eq-grey uppercase">Result</dt>
            <dd className="text-eq-ink mt-1">{formatNsxTestResult(test.overall_result)}</dd>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-gray-200">
          ...
          <div className="grid grid-cols-3 gap-3 text-sm">
            ...
            <div>
              <dt className="text-xs text-eq-grey">Rating</dt>
              <dd className="text-eq-ink mt-0.5">{test.cb_rating ?? '—'}</dd>
            </div>
            <div>
              <dt className="text-xs text-eq-grey">Poles</dt>
              <dd className="text-eq-ink mt-0.5">{test.cb_poles ?? '—'}</dd>
            </div>
            <div>
              <dt className="text-xs text-eq-grey">Trip Unit</dt>
              <dd className="text-eq-ink mt-0.5">{test.trip_unit ?? '—'}</dd>
            </div>
          </div>
        </div>
