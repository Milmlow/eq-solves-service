...
import { createNsxTestAction, updateNsxTestAction } from './actions'
import type { NsxTest, Asset, Site, Profile } from '@/lib/types'

interface NsxTestFormProps {
  open: boolean
  onClose: () => void
  test?: NsxTest | null
...
export function NsxTestForm({ open, onClose, test, assets, sites, technicians }: NsxTestFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = isEdit
      ? await updateNsxTestAction(test!.id, formData)
      : await createNsxTestAction(formData)
  ...
  return (
    <SlidePanel open={open} onClose={handleClose} title={isEdit ? 'Edit NSX Test' : 'Add NSX Test'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <FormInput label="CB Make" name="cb_make" defaultValue={test?.cb_make ?? ''} placeholder="e.g. Schneider" />
        <FormInput label="CB Model" name="cb_model" defaultValue={test?.cb_model ?? ''} placeholder="e.g. NSX400N" />
        <FormInput label="CB Serial" name="cb_serial" defaultValue={test?.cb_serial ?? ''} placeholder="Serial number" />
        <div className="grid grid-cols-2 gap-3">
          <FormInput label="Rating" name="cb_rating" defaultValue={test?.cb_rating ?? ''} placeholder="e.g. 400A" />
          <FormInput label="Poles" name="cb_poles" defaultValue={test?.cb_poles ?? ''} placeholder="e.g. 3P, 4P" />
        </div>
        <FormInput label="Trip Unit" name="trip_unit" defaultValue={test?.trip_unit ?? ''} placeholder="e.g. Micrologic 5.3E" />
