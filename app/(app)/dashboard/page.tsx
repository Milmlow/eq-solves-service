...
export default async function DashboardPage({
  ...
  if (user) {
    ...
    const [{ data: profile }, { data: membership }] = await Promise.all([
      ...
      supabase
        .from('tenant_members')
        // tech_onboarded_at added in migration 0102 — autogenerated types
        // don't include it yet, so we cast on read. Once db types
        // regenerate, the cast can go.
        .select('role, tenant_id, tech_onboarded_at' as 'role, tenant_id')
        .eq('user_id', user.id)
        .eq('is_active', true)
        .limit(1)
        .maybeSingle(),
    ...
    const memberRow = membership as { role?: string; tenant_id?: string; tech_onboarded_at?: string | null } | null
