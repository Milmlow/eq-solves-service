...
export default async function DashboardPage({
  ...
  // ── Auth: JWT fast path (Shell iframe / ehow) OR Supabase session ──
  //
  // ehow (sks-canonical, ehowgjardagevnrluult) has no profiles or
  // tenant_members tables — the standard Supabase session path would
  // always produce tenantId=null and show "workspace not set up".
  //
  // When Shell SSO sets eq_service_jwt, we use createJwtClient so that
  // RLS policies (which read auth.jwt().app_metadata.tenant_id) evaluate
  // against the JWT's tenant_id — the same UUID embedded in every ehow
  // data row. Identity and role come from the JWT claims directly.
  ...
  const supabase = hasJwtSession && serviceJwtRaw
    ? createJwtClient(serviceJwtRaw)
    : await createClient()
  ...
  if (hasJwtSession && jwtClaims) {
    // JWT path: Shell iframe via eq_service_jwt. No profiles / tenant_members
    // on ehow — read identity and role directly from JWT claims.
    userId = jwtClaims.sub
    userRole = (jwtClaims.app_metadata.eq_role as Role) ?? 'apprentice'
    tenantId = jwtClaims.app_metadata.tenant_id!
    // userName stays 'there' — no profiles table on ehow.
    // techOnboardedAt / setupChecklistDismissedAt stay null — no tenant_members.
  } else {
